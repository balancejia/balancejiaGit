...
(function(){
	var Provider, aFilters = [], nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sCellFileTemplate = '<b class="stream-uploading-ico"></b>' +
					...
					+
					'<img width="40px;" height="40px;" src="" style="float:left;margin-right:5px;">'+
					'<div class="stream-file-name" ><strong></strong></div>'
					+'<a class="stream-cancel"  href="javascript:void(0)" onclick="delData(this);">删除</a>'
					+'<div id="stream-total"></div>' +
	...
	return ext+".gif";
	 ...
	 function formatBytes(size) {
		if(isNaN(size)){// zili 形参size单位是Byte
			return 0+'B';
		}
	    else if (size < 1024) {
			return (size + 'B');
	...
	function fShowMessage(msg, warning) {
		var o = document.getElementById("fShowMsg");
	...
	SWFProvider.prototype = {
		...
		upload : function(uploader, url, postVars) {//F3
	...
	SWFUploader.prototype = {
		...
		startUpload : function(url, postVars, fileFieldName) {//F4  
			if (this.get("uploader")) {//flash的上传监控之处
				...
				uploader.callSWF("upload", [id, url, postVars, fileFieldName]);//flash上传版本调用后台servlet之处
			}
		},
	...
	StreamProvider.prototype = {
		...
		upload : function(uploader, url, postVars) {//zili 三
	...
	StreamUploader.prototype = {
		...
		formUpload : function() {//这段代码并没有调用过
			this.resetXhr();
		...
		uploadEventHandler: function(event){//html5上传监控的事件
			...
			switch(event.type){
				case "load":
					...
					if(uploaded < this.get("size") -1) {
						...
						this.streamUpload(uploaded);//zili html5每次10M请求服务器
					} else {
		...
		startUpload: function(url, postVars, fileFieldName){//zili
			this.fileStartPosValue = null;
			this.retriedTimes = 0;
	...
	function Main(cfg){//初始化配置
		...
		this.uploadInfo = {};
		this.config = {
			//delHandler:cfg.delHandler,//删除方法
		    showImg:cfg.showImg,
			basePath:cfg.basePath,//zili
			miniUpload:cfg.miniUpload,//zili
			enabled : !0,
	...
	Main.prototype = {
		...
		initializer : function() {
			...
			if (!this.config.customered) {
				...
				if(!this.config.miniUpload){//zili
				  fAddClass(filesQueuePanel, "stream-main-upload-box");
				}else{
				 fAddClass(filesQueuePanel, "stream-main-upload-box-mini");
				}
			    ...
			    if(this.config.miniUpload){//zili mini
					$(filesQueue).css('height', '');
				}
				filesQueuePanel.appendChild(filesQueue);
			//zili	filesQueuePanel.appendChild(totalQueue);
				...
				$("#"+sStreamMessagerId).append('<div id="fShowMsg"></div>');//zili
			}
		...
		renderUI : function(file_id) {//页面样子绑定
			$("#fShowMsg").html("");//zili
			var progressNode = this.uploadInfo[file_id].progressNode,
				cellInfosNode = this.uploadInfo[file_id].cellInfosNode,
				size = formatBytes(this.uploadInfo[file_id].file.get("size")),//zili
				name = this.uploadInfo[file_id].file.get("name"),//zili
			...
			//zili this.getNode("stream-uploaded", cellInfosNode).innerHTML = "0/" + total;
			this.getNode("stream-uploaded", cellInfosNode).innerHTML = total;
			var _total = formatBytes(this.totalFileSize);
			/*选择后显示图片 zili*/
			var ext = -1 !== name.indexOf(".") ? name.replace(/.*[.]/, "").toLowerCase() : "";
			var showImg = "";
			if(!this.config.miniUpload){
				showImg = whatImgShow(ext);
			}else{
				if(!this.config.showImg){
				showImg = "annex.png";
				$("#"+file_id).find("img").attr("height","16px");
				$("#"+file_id).find("img").attr("width","16px");
				$("#"+file_id).find(".stream-cell-file").attr("class","stream-cell-file-mini");
				$("#"+file_id).find(".file-name").attr("class","file-name-mini");
				}else{
					showImg = whatImgShow(ext);
				}
			}
			$("#"+file_id).find("img").attr("src",this.config.basePath+'/script/stream/css/img/'+showImg);
			
			//zili this.getNode("_stream-total-size", this.totalContainerPanel).innerHTML = _total;
		},
		bindUI : function(file_id) {
			//--TODO
			var b = this.uploadInfo[file_id].progressNode,cancelBtn = this.getNode("stream-cancel", b);
			//cancelBtn = $(b).next("a.stream-cancel");
		...
		completeUpload : function(info) {//H7
			//zili   上传成功将保存至数据库的id给其赋值
			var file_id = eval("(" + info.msg + ")").fileId;
			this.data_file_id = file_id ;
		...
		onSelect : function(list) {
			//z fShowMessage("selected files: " + list.length);
		},
		onFileCountExceed : function(selected, limit) {
			 fShowMessage("您只能选择" + limit+"个文件", true);
		},
		onMaxSizeExceed : function(file) {
			 fShowMessage("您选择的:" + file.name + " 大小为:" + formatBytes(file.size) +" 超出了" + formatBytes(file.limitSize), true);
		},
		onExtNameMismatch: function(file) {
			 fShowMessage("只允许上传[" + file.filters.toString() + "]文件 ", true);
		},
		onAddTask: function(file) {
			 fShowMessage("Add to task << name: [" + file.name);
		},
		onCancel : function(file) {
			//z fShowMessage("Canceled: " + file.name);
		},
		onStop : function() {
			//z fShowMessage("Stopped!");
		},
		onCancelAll : function(numbers) {
			//z fShowMessage(numbers + " files Canceled! ");
		},
		onComplete : function(file) {
			//z fShowMessage("File:" + file.name + ", Size:" + file.size+" onComplete	[OK]");
		},
		onQueueComplete : function(msg) {
			//z fShowMessage("onQueueComplete(msg: "+msg+")	---==>		[OK]");
		},
		onUploadError : function(status, msg) {
			//z fShowMessage("Error Occur.  Status:" + status + ", Message: " + msg, true);
			fShowMessage("服务器异常", true);
		},
		disable : function() {
		...
		cancelOne : function(file_id, stopping) {
			...
			var totalSize = this.totalFileSize - this.uploadInfo[file_id].file.config.size, info = {
				...
				formatTotalLoaded: formatBytes(this.totalUploadedSize),
			...
			var _loaded = formatBytes(this.totalUploadedSize);
		...
		upload : function(index) {//H1 F1
			...
			xhr.onreadystatechange = function() {
				...
				try {
					...
					if (token) {
						...
						: self.uploadFile(file, frmUploadURL, token, "formUpload");/*zili 判断调用html5还是flash*/
					} else {
						/** not found any token */
			...
			xhr.send();//zili 调用tokenServlet方法，用于生成临时文件
		},
		uploadFile : function(file, url, token, method) {//zili H2 F2
			var token = {
				token : token,
		...
		uploadProgress : function(a) {//进度条生成  上传速度
			...
			if (this.config.customered) {
				var info = {
					...
					formatLoaded:       formatBytes(a.bytesLoaded),
					speed:              a.bytesSpeed,
					formatSpeed:        this.formatSpeed(a.bytesSpeed),
					size:               a.bytesTotal,
					formatSize:         formatBytes(a.bytesTotal),
					percent:            percent,
					timeLeft:           a.remainTime,
					formatTimeLeft:     this.formatTime(a.remainTime),
					totalSize:          this.totalFileSize,
					formatTotalSize:    formatBytes(this.totalFileSize),
					totalLoaded:        this.totalUploadedSize + a.bytesLoaded,
					formatTotalLoaded:  formatBytes(this.totalUploadedSize + a.bytesLoaded),
			...
			var progressNode = this.uploadInfo[id].progressNode,
				...
				c = this.formatSpeed(a.bytesSpeed), loaded = formatBytes(bytesLoaded),
				total = formatBytes(a.bytesTotal), _remainTime = this.formatTime(a.remainTime),
			...
			//zili this.getNode("stream-uploaded", cellInfosNode).innerHTML = loaded + "/" + total;
			this.getNode("stream-uploaded", cellInfosNode).innerHTML = total;
			var _loaded = formatBytes(this.totalUploadedSize + bytesLoaded);
		...
		uploadComplete : function(a) {//H5  F5  服务器端上传完文件后调用
			...
			var info = {
				...
				formatLoaded:       formatBytes(a.target.get("size")),
				size:               a.target.get("size"),
				formatSize:         formatBytes(a.target.get("size")),
				percent:            100,
				totalSize:          this.totalFileSize,
				formatTotalSize:    formatBytes(this.totalFileSize),
				totalLoaded:        this.totalUploadedSize,
			...
			if (!this.config.customered) {
				var progressNode = this.uploadInfo[id].progressNode,
					...
					size = a.target.get("size"), a = eval("(" + a.data + ")"), fmtSize = formatBytes(size);
				...
				//zili this.getNode("stream-cancel", progressNode).innerHTML = "";
				//zili fAddEventListener(this.getNode("stream-cancel", progressNode),"click", this.config.delHandler);
				var _loaded = formatBytes(this.totalUploadedSize);
				var percent = this.totalUploadedSize * 10000 / this.totalFileSize / 100;
				100 > percent && (percent = parseFloat(percent).toFixed(2));
				if (this.totalFileSize === 0)
		...
		validateFile : function(uploader) {
			var name = uploader.get("name"), size = uploader.get("size"),
				...
				info = {
					...
					formatSize:       formatBytes(uploader.get("size")),//zili  this.formatBytes(uploader.get("size")),
					lastModifiedDate: uploader.get("lastModifiedDate"),
					limitSize:        this.get("maxSize"),
					formatLimitSize:  formatBytes(this.get("maxSize")),//zili this.formatBytes(this.get("maxSize")),
					filters:          filters
				};
			...
			if (this.get("maxSize") < size){
				...
				fShowMessage("您上传的文件"+name+"大小为 0 字节，请检查后重试。",true);
				return !1;
			}
			else {
		...
		formatSpeed : function(a) {
			...
			1024 <= Math.round(a / 1024) 
				? (b = Math.round(100 * (a / 1048576)) / 100, b = Math.max(0, b), b = isNaN(b) ? 0 : parseFloat(b).toFixed(1), a = b + "MB/s")
				: (b = Math.round(100 * (a / 1024))	/ 100, b = Math.max(0, b), b = isNaN(b) ? 0 : parseFloat(b).toFixed(1), a = b + "KB/s");
			return a;
		},
	...
	var bFileSlice = !1;
	var bStreaming = function() {//判断浏览器是否支持html5
